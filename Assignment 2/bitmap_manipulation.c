...
int main(void) {
	...
	char header2[8];
	fread(header2, 1, 8, infile1);

	int imageSize;
	fread(&imageSize, sizeof(int), 1, infile1);
	printf("Image Size: %d\n", imageSize);

	char header3[16]; // bytes after the image size piece
	fread(header3, 1, 16, infile1);
	...
	fwrite(header2, 1, 8, outfile1);
	fwrite(&imageSize, sizeof(int), 1, outfile1);
	fwrite(header3, 1, 16, outfile1);
